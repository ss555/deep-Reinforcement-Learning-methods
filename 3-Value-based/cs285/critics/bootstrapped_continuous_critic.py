...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        reward_n = ptu.from_numpy(reward_n)#.unsqueeze(1)
        terminal_n = ptu.from_numpy(terminal_n)
        for i in range(self.num_target_updates):
            # target = self.gamma*ptu.from_numpy(self.forward_np(next_ob_no)*(1-terminal_n)+reward_n
            # torch.autograd.set_detect_anomaly(True)
            with torch.no_grad():
            # BAD    target=(self.gamma*self.critic_network.forward(next_ob_no).squeeze(1)*(1-terminal_n)).unsqueeze(1)+reward_n
                target=self.gamma*self.forward(next_ob_no)*(1-terminal_n)+reward_n
            for i in range(self.num_grad_steps_per_target_update):
                v_s_n = self.forward(ob_no)
                # self.optimizer.zero_grad()
                loss = self.loss(v_s_n, target)
                self.optimizer.zero_grad()
                loss.backward()
                self.optimizer.step()
