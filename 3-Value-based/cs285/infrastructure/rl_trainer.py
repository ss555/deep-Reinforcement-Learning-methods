...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(self, itr, initial_expertdata, collect_policy, num_transitions_to_sample, save_expert_data_to_disk=False):
        ...
        env_steps_video=0
        if self.logvideo:
            train_videos_path = utils.sample_n_trajectories(self.env, collect_policy, MAX_NVIDEO, MAX_VIDEO_LEN)
        batch_size = num_transitions_to_sample
        paths, envs_steps_batch = utils.sample_trajectories(self.env, collect_policy, batch_size, self.params['ep_len'])
        return paths, envs_steps_batch, train_video_paths

    def train_agent(self):
        all_logs=[]
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ob_batch,ac_batch,rews,next_obs,terminals=self.agent.sample(self.params['train_batch_size'])
            train_logs=self.agent.train(ob_batch,ac_batch,rews,next_obs,terminals)
