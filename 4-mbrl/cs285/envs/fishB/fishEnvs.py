...
class FishStationary(gym.Env):
    ...
    def get_reward(self, obs, act_sequence):
        Fy=obs[:,0].copy()
        Fy_dot=obs[:,1].copy()
        act=obs[:,2].copy()
    ...
    def step(self, action):
        ...
        # assert self.action_space.contains(np.array(action,dtype=np.float32)), err_msg
        ...
        # translate action
        phi_c = np.deg2rad(self.Gamma*action)
        act = phi_c
        # Physical model
        ### Servo motor
        ...
        self.state = np.array([Fy_mesure, Fy_dot, act], dtype=np.float64)  # QA: np.float16!?
