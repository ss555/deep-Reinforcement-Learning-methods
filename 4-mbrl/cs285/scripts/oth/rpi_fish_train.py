...
from linear_expe import make_red_yellow_env_speed
...
class MBPO_Trainer(object):
    def __init__(self, params, trial=None):
        ...
        self.params['env_name'] = LoggerWrap(FishMoving()) #gym.make('FishMoving-v0')
        # self.params['env_name'] = gym.make(params['env_name'])
...
def main():
    ...
    parser.add_argument('--batch_size_initial', type=int,
                        default=20000)  # (random) steps collected on 1st iteration (put into replay buffer)
    parser.add_argument('--batch_size', type=int,
                        default=8000)  # steps collected per train iteration (put into replay buffer)
    ...
    logdir = logdir_prefix + args.exp_name + '_' + str(args.env_name) + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
