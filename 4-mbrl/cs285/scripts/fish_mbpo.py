...
from rlutils.envs import FishMoving
...
def main():
    ...
    # Common parameters
    parser.add_argument('--env_name', type=str, default='FishMoving-v00')  # reacher-cs285-v0, ant-cs285-v0, cheetah-cs285-v0, obstacles-cs285-v0
    ...
    parser.add_argument('--train_batch_size', '-tb', type=int,
                        default=512)  # steps used per gradient step (used for training)
    ...
    parser.add_argument('--num_agent_train_steps_per_iter', type=int, default=2000)
    parser.add_argument('--batch_size_initial', type=int,
                        default=5000)  # (random) steps collected on 1st iteration (put into replay buffer)
    parser.add_argument('--batch_size', type=int,
                        default=5000)  # steps collected per train iteration (put into replay buffer)
    ...
    parser.add_argument('--sac_batch_size', type=int, default=1500)  # steps collected per train iteration
    parser.add_argument('--sac_discount', type=float, default=0.99)
    parser.add_argument('--sac_init_temperature', type=float, default=0.1)
    parser.add_argument('--sac_learning_rate', type=float, default=3e-4)
    parser.add_argument('--sac_n_layers', type=int, default=2)
    parser.add_argument('--sac_size', type=int, default=256)
    parser.add_argument('--sac_n_iter', type=int, default=5000)

    # MBPO parameters
    parser.add_argument('--mbpo_rollout_length', type=int, default=10)
    args = parser.parse_args()

    # convert to dictionary
    params = vars(args)

    monitor_dir, _ = make_dir_exp(
        os.path.abspath(os.path.join(os.path.dirname(__file__), './logs')))  # '../docs/weightsParams/ppo.yml')
    ...
    # params['env']=env
    #######################GYM ENV#######################
    params['env']= TimeLimit(FishMoving(),768) #gym.make(params['env_name'])
    params['logdir'] = monitor_dir
    ###################
    ### RUN TRAINING
    ###################
