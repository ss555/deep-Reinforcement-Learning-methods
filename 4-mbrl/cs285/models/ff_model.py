...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        delta_pred_normalized = self.delta_network.forward(concatenated_input)# TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        predictions = self.forward(obs, acs, data_statistics['obs_mean'], \
        ...
        # prediction = obs+unnormalize(ptu.to_numpy(self.delta_network(torch.cat([ptu.from_numpy(obs),ptu.from_numpy(acs)],dim=1))),data_statistics['delta_mean'],data_statistics['delta_std'])# TODO(Q1) get numpy array of the predicted next-states (s_t+1)
        # prediction = obs+unnormalize(self.delta_network(obs,acs),data_statistics['delta_mean'],data_statistics['delta_std'])# TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :param data_statistics: A dictionary with the following keys (each with a numpy array as the value):
             - 'obs_mean'
             - 'obs_std'
             - 'acs_mean'
             - 'acs_std'
        ...
        :return:
        """
        delta = self.forward(observations, actions, data_statistics['obs_mean'], \
                             data_statistics['obs_std'], data_statistics['acs_mean'], data_statistics['acs_std'], \
                             data_statistics['delta_mean'], data_statistics['delta_std'])[1]
        target = ptu.from_numpy(normalize(next_observations - observations, data_statistics['delta_mean'],
                                          data_statistics['delta_std']))  # .detach() # TODO(Q1) compute the normalized target for the model.
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.

        loss = self.loss(delta, target)  # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
