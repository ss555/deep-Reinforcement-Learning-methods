'''
optuna file for hyperparameter tuning of MBPO with fish moving environment
'''
...
def sample_mb(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    mpc_horizon = trial.suggest_int("mpc_horizon", 1, 40)
    ...
    batch_size_initial = trial.suggest_categorical("batch_size_initial", [int(1e3), int(768), int(1e4), int(5e3)])
    ...
    cem_num_elites = "auto"
    num_agent_train_steps_per_iter = trial.suggest_categorical("num_agent_train_steps_per_iter", [1000, 768, 5000, 2000])
...
with open(os.path.join(os.path.dirname(__file__),"./../params/mbpo.yml"), "r") as f:
...
def objective(trial: optuna.Trial) -> float:
    ...
    # kwargs['agent_params']['sac_params'].update(sample_sac(trial))
    ...
    print(kwargs['env_name'])
    logdir = logdir_prefix + '_optuna' + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
    logdir = os.path.join(data_path, logdir)


    trainer = MBPO_Trainer(kwargs,trial=trial)#,logdir=logdir)
    ...
    if trainer.rl_trainer.is_pruned:
