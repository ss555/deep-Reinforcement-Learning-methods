...
class PpoMbAgent(BaseAgent):
    def __init__(self, env, agent_params):
        super(PpoMbAgent, self).__init__()
        self.mb_agent = MBAgent(env, agent_params)
        self.ppo = PPO(env, agent_params['sac_params'])
        self.env = env
    ...
    def collect_model_trajectory(self, rollout_length=1):
        ...
        for _ in range(rollout_length):
            ...
            ac = self.ppo.actor.get_action(ob)
    ...
    def add_to_replay_buffer(self, paths, from_model=False, **kwargs):
        self.ppo.add_to_replay_buffer(paths)
